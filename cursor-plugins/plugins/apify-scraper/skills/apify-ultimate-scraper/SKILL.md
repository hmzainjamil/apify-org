...
description: Universal AI-powered web scraper for any platform. Scrape data from Instagram, Facebook, TikTok, YouTube, Google Maps, Google Search, Google Trends, Booking.com, TripAdvisor, Amazon, Walmart, eBay, IKEA, and 50+ more. Use for lead generation, brand monitoring, competitor analysis, influencer discovery, trend research, content analytics, audience analysis, e-commerce pricing, reviews, or any data extraction task.
---
...
| Actor ID | Best For |
|----------|----------|
| `apify/e-commerce-scraping-tool` | Product data, pricing, reviews, and sellers across Amazon, Walmart, eBay, IKEA, and 50+ marketplaces |
...
| **E-commerce: Products & Pricing** | `apify/e-commerce-scraping-tool` with `keyword` + `marketplaces` or `detailsUrls` |
| **E-commerce: Reviews** | `apify/e-commerce-scraping-tool` with `reviewListingUrls` or `keywordReviews` + `marketplacesReviews` |
| **E-commerce: Seller Intelligence** | `apify/e-commerce-scraping-tool` with `googleShoppingSearchKeyword` + `scrapeSellersFromGoogleShopping` |

---

#### E-commerce Workflows

For e-commerce tasks, use `apify/e-commerce-scraping-tool` with these input patterns:

**Products & Pricing** (price monitoring, MAP compliance, competitor analysis):
```json
{
...
Optional AI summary fields: `fieldsToAnalyze` (e.g. `["name", "offers", "brand"]`) and `customPrompt`.

**Customer Reviews** (sentiment analysis, brand perception, quality issues):
```json
{
  "reviewListingUrls": [
    {"url": "https://www.amazon.com/dp/B09V3KXJPB"}
...
Sort options: `Most recent`, `Most relevant`, `Most helpful`, `Highest rated`, `Lowest rated`.

**Seller Intelligence** (unauthorized resellers, vendor discovery via Google Shopping):
```json
{
...
Amazon (20+ regions), Walmart, Costco, Home Depot, Allegro, Alza, Kaufland, Cdiscount, IKEA (40+ country/language combinations), Google Shopping.
...
| E-commerce products | Analyze reviews or find sellers |
| E-commerce reviews | Cross-reference with competitor products |
| Seller data | Verify with direct product URL lookups |
